#ifndef OUTBOX_INTERNAL_H
#define OUTBOX_INTERNAL_H
...
#define OUTBOX_CATEGORY_CAPACITY 96u
#define OUTBOX_CATEGORY_SLOT_BYTES 128u
...
#define OUTBOX_CURSOR_TOKEN_CAPACITY 48u
#define OUTBOX_CURSOR_DIRECTORY_NAME "cursors"
#define OUTBOX_PROVIDER_ID_CAPACITY 64u
#define OUTBOX_MAX_PROVIDER_CURSORS 8u
#define OUTBOX_STATS_FIELD_COUNT 13u
#if defined(__powerpc64__) || defined(__ppc64__) || defined(__PPC64__)
#define OUTBOX_CACHELINE 128u
#else
#define OUTBOX_CACHELINE 64u
#endif

typedef struct outbox_slot_t {
  ...
  _Alignas(OUTBOX_CACHELINE) _Atomic uint64_t state_sequence;
  uint64_t sequence;
...
typedef struct outbox_provider_cursor_t {
  uint32_t active;
  char provider_id[OUTBOX_PROVIDER_ID_CAPACITY];
  char* cursor_file_path;
  uint64_t ack_segment_id;
  uint64_t ack_offset;
} outbox_provider_cursor_t;
...
typedef struct outbox_t {
  ...
  outbox_provider_cursor_t provider_cursors[OUTBOX_MAX_PROVIDER_CURSORS];
  outbox_slot_t* slots;
  char* categories;
  char* payloads;
  ...
  _Alignas(OUTBOX_CACHELINE) _Atomic uint64_t enqueue_cursor;
  _Alignas(OUTBOX_CACHELINE) _Atomic uint64_t dequeue_cursor;
  _Alignas(OUTBOX_CACHELINE) _Atomic uint32_t active_producer_count;
