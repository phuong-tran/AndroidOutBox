...
static int segment_file_name(uint64_t segment_id, char* buffer, size_t buffer_capacity) {
  return snprintf(buffer,
                  buffer_capacity,
                  "%s%020llu%s",
                  OUTBOX_SEGMENT_PREFIX,
                  (unsigned long long)segment_id,
                  OUTBOX_SEGMENT_SUFFIX);
}
...
static int parse_segment_id(const char* file_name, uint64_t* out_segment_id) {
  ...
  if (sscanf(file_name,
             OUTBOX_SEGMENT_PREFIX "%llu" OUTBOX_SEGMENT_SUFFIX "%c",
             &segment_id,
             &trailing) != 1) {
    return 0;
  }
...
char* outbox_spool_segment_path(const char* directory, uint64_t segment_id) {
  char file_name[OUTBOX_SEGMENT_NAME_CAPACITY];
...
char* outbox_spool_cursor_directory_path(const char* directory) {
  return path_join(directory, OUTBOX_CURSOR_DIRECTORY_NAME);
}
...
int outbox_spool_provider_id_is_valid(const char* provider_id) {
  ...
  while (provider_id[index] != '\0') {
    ...
    if (index >= OUTBOX_PROVIDER_ID_CAPACITY - 1u ||
...
char* outbox_spool_provider_cursor_path(
  ...
  char file_name[OUTBOX_PROVIDER_ID_CAPACITY + 8u] = {};
  if (cursor_directory == NULL ||
      !outbox_spool_provider_id_is_valid(provider_id)) {
    return NULL;
  }
...
void outbox_spool_load_cursor_locked(
  ...
  char buffer[OUTBOX_CURSOR_TOKEN_CAPACITY] = {};
  int fd = -1;
  ssize_t bytes_read = 0;

  if (outbox == NULL || cursor == NULL) {
    return;
  }

  cursor->ack_segment_id = segment_count == 0u ? outbox->active_segment_id : min_segment_id;
  ...
  if (!outbox_spool_parse_cursor_token(buffer,
  ...
  if (cursor->ack_segment_id > outbox->active_segment_id) {
    cursor->ack_segment_id = outbox->active_segment_id;
    cursor->ack_offset = 0u;
  }
}
...
outbox_provider_cursor_t* outbox_spool_get_provider_cursor_locked(
  ...
  outbox_provider_cursor_t* cursor = NULL;
  ...
  for (index = 0u; index < outbox->provider_cursor_count; ++index) {
    if (outbox->provider_cursors[index].active != 0u &&
        strcmp(outbox->provider_cursors[index].provider_id, provider_id) == 0) {
      return &outbox->provider_cursors[index];
    }
  }
  if (create_if_missing == 0 ||
      outbox->provider_cursor_count >= OUTBOX_MAX_PROVIDER_CURSORS) {
    return NULL;
  }
  ...
  cursor = &outbox->provider_cursors[outbox->provider_cursor_count];
  memset(cursor, 0, sizeof(*cursor));
  cursor->active = 1u;
  snprintf(cursor->provider_id, sizeof(cursor->provider_id), "%s", provider_id);
  cursor->cursor_file_path =
      outbox_spool_provider_cursor_path(outbox->cursor_directory_path, provider_id);
  if (cursor->cursor_file_path == NULL ||
      !outbox_spool_scan_segment_bounds(outbox->spool_directory_path,
                                                      &min_segment_id,
                                                      &max_segment_id,
                                                      &segment_count)) {
  ...
  outbox_spool_load_cursor_locked(outbox, cursor, min_segment_id, segment_count);
  outbox->provider_cursor_count += 1u;
  return cursor;
}
...
int outbox_spool_cleanup_old_segments_locked(
    outbox_t* outbox) {
  const uint32_t max_segment_count = outbox->max_archived_segments + 1u;
  uint64_t min_segment_id = 0u;
  uint64_t max_segment_id = 0u;
  uint32_t segment_count = 0u;
  ...
  while (outbox_spool_scan_segment_bounds(outbox->spool_directory_path,
                                                        &min_segment_id,
                                                        &max_segment_id,
                                                        &segment_count) &&
         segment_count > max_segment_count &&
         min_segment_id < outbox->active_segment_id) {
    char* oldest_path =
        outbox_spool_segment_path(outbox->spool_directory_path, min_segment_id);
    if (oldest_path == NULL) {
      return 0;
    }
    ...
    for (uint32_t index = 0u; index < outbox->provider_cursor_count; ++index) {
      outbox_provider_cursor_t* cursor = &outbox->provider_cursors[index];
...
int outbox_spool_has_unacked_records_locked(
  ...
  if (outbox == NULL ||
      atomic_load_explicit(&outbox->started, memory_order_acquire) == 0u ||
      outbox->spool_directory_path == NULL ||
      outbox->active_segment_id == 0u) {
    return 0;
  }

  for (provider_index = 0u; provider_index < outbox->provider_cursor_count; ++provider_index) {
    const outbox_provider_cursor_t* cursor = &outbox->provider_cursors[provider_index];
    uint64_t segment_id = 0u;
    if (cursor->active == 0u || cursor->ack_segment_id == 0u) {
      continue;
    }
    for (segment_id = cursor->ack_segment_id;
         segment_id <= outbox->active_segment_id;
         ++segment_id) {
      char* path = outbox_spool_segment_path(outbox->spool_directory_path, segment_id);
      uint64_t size = 0u;
      if (path == NULL) {
        return 0;
      }
      size = outbox_spool_file_size_bytes(path);
      free(path);
...
int outbox_spool_rotate_active_segment_locked(
  ...
  char* next_path = NULL;
  int next_fd = -1;
  const uint64_t next_segment_id = outbox->active_segment_id + 1u;
  next_path =
      outbox_spool_segment_path(outbox->spool_directory_path, next_segment_id);
  if (next_path == NULL) {
    return 0;
  }
  ...
  outbox->active_segment_id = next_segment_id;
  outbox->active_file_path = next_path;
  outbox->active_fd = next_fd;
  outbox->current_file_size_bytes = 0u;
  outbox->roll_count += 1u;
  return outbox_spool_cleanup_old_segments_locked(outbox);
}
...
outbox_status_t outbox_read_next_batch(
  ...
  if (atomic_load_explicit(&outbox_state.started, memory_order_acquire) == 0u ||
      outbox_state.spool_directory_path == NULL) {
  ...
  cursor = outbox_spool_get_provider_cursor_locked(&outbox_state,
                                                                 provider_id,
                                                                 1);
  if (cursor == NULL) {
    pthread_mutex_unlock(&outbox_state.mutex);
  ...
  active_segment_id = outbox_state.active_segment_id;

  while (record_count < max_records && total_bytes < max_bytes && segment_id <= active_segment_id) {
    char* path = outbox_spool_segment_path(outbox_state.spool_directory_path, segment_id);
    FILE* file = NULL;
    char* line = NULL;
    ...
    while (record_count < max_records && total_bytes < max_bytes &&
      ...
      if (record == NULL) {
        ...
        fclose(file);
        pthread_mutex_unlock(&outbox_state.mutex);
        outbox_record_batch_t partial = {
            .records = records,
            .record_count = record_count,
        };
        outbox_free_record_batch(&partial);
        return OUTBOX_STATUS_INTERNAL_ERROR;
      }
  ...
  if (out_batch->ack_token == NULL) {
    pthread_mutex_unlock(&outbox_state.mutex);
    outbox_free_record_batch(out_batch);
    return OUTBOX_STATUS_INTERNAL_ERROR;
  }
  pthread_mutex_unlock(&outbox_state.mutex);
  return OUTBOX_STATUS_OK;
...
outbox_status_t outbox_ack(
  ...
  if (provider_id == NULL ||
      !outbox_spool_parse_cursor_token(ack_token, &segment_id, &offset)) {
    return OUTBOX_STATUS_INVALID_ARGUMENT;
  }
  ...
  if (segment_id > outbox_state.active_segment_id) {
    pthread_mutex_unlock(&outbox_state.mutex);
    return OUTBOX_STATUS_INVALID_ARGUMENT;
  }

  path = outbox_spool_segment_path(outbox_state.spool_directory_path, segment_id);
  if (path == NULL) {
    pthread_mutex_unlock(&outbox_state.mutex);
    return OUTBOX_STATUS_INTERNAL_ERROR;
  }
  ...
  has_more_records = outbox_spool_has_unacked_records_locked(&outbox_state);
  pthread_mutex_unlock(&outbox_state.mutex);
